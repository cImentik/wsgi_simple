...
"""
Implemets a render for templates.

Replaces special lexemes in templates. 
It builds a tree of tokens for replacement by expressions. 
Can work with "if" and "for" expressions only
...
TOK_REGEX = re.compile(r"({0}.*?{1}|{2}.*?{3})".format(
    VAR_TOKEN_START,
    VAR_TOKEN_END,
    BLOCK_TOKEN_START,
    BLOCK_TOKEN_END
...
class TemplateSyntaxError(TemplateError):
    ...
    def __str__(self):
        return "{0} seems like invalid syntax".format(self.error_syntax)
